...
def create_driver():
    ...
    wait_big = WebDriverWait(driver, 30)
    return driver, wait, wait_big
...
def browser_open(url, page):
    ...
    subprocess.call(['google-chrome', '--no-sandbox', '--hide-crash-restore-bubble', '--remote-debugging-port=9222', url], stdout=open(os.devnull, "w"), stderr=subprocess.STDOUT)
    # command = f'start chrome --hide-crash-restore-bubble --remote-debugging-port=9222 "{url}"'
    # os.system(command)


def solve_hcaptcha(driver, wait, wait_big):
    def solvehCaptcha():
...
def get_links(street, state):
    ...
    driver, wait, wait_big = create_driver()
    if db_state_dict[1] == 2:
        solve_hcaptcha(driver, wait, wait_big)
        requests.put("http://localhost:5000/id/1")
...
def open_persons(url, street, name, address, browser):
    ...
    #     try:
    #         age_obj = person_soup.find('div', {'class', 'row pl-md-2'}).find('span').text
    #         age = age_obj.split("(")[0].split('Age')[1]
    #     except:
    #         age = person_soup.find('div', {'class', 'row pl-md-2'}).find('span').text

    #     mail_exists = person_soup.findAll('i', {'class': 'fa fa-envelope text-center'})
    #     if len(mail_exists) > 0:
    #         sections = person_soup.find_all('div', {'class', 'row pl-md-3'})
    #         for section in sections:
    #             if section.find('i', {'class': 'fa fa-envelope text-center'}):
    #                 email_list = []
    #                 emails = section.findAll('div', {'class', 'row pl-sm-2'})
    #                 for email in emails:
    ...
    #     print(f"Name: {name}, Address: {address}, Age: {age}, Wireless Numbers: {wireless_numbers}, Landline Numbers: {landline_numbers}, Emails: {mail_list}")
